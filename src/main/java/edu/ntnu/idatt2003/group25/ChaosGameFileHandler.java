...
public class ChaosGameFileHandler extends ChaosGameDescription{
  ...
  public ChaosGameDescription readFromFile(String path) throws FileNotFoundException {
    try {
      ...
      //leser en og en linje fra fil ved static scanner i egen klasse
      // sjekker første linje -> Julia (behandler info om Julia) -> Affine (behandler affine)
      // finner max og min, samt setter alle transformasjonen inn i transform ArrayList<>
      //return new ChaosGameDescription(Transform2D,max, min);
    return null;
    }
